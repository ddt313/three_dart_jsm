  ...
  late int innerHeight;
	...
	parseParameters(Map materialNode, textureMap, ID ) {
		...
		if ( materialNode["Diffuse"] != null ) {
			...
			parameters["color"] = new Color().fromArray( List<double>.from(materialNode["DiffuseColor"]["value"] ));
		...
		if ( materialNode["Specular"] != null ) {
			...
			parameters["specular"] = new Color().fromArray( List<double>.from(materialNode["SpecularColor"]["value"] ));
			...
			if ( geoInfo["skeleton"] != null ) {
				...
				if ( weights.length > 4 ) {
					...
					if ( ! displayedWeightsWarning ) {
						...
						print( 'THREE.FBXLoader: Vertex has more than 4 skinning weights assigned to vertex. Deleting additional weights.' );
...
class AnimationParser {
	...
	addClip( rawClip ) {
		...
		return new AnimationClip( rawClip["name"], - 1, List<KeyframeTrack>.from(tracks) );
	}
...
isFbxFormatASCII(String text ) {
	...
	read( offset ) {
		...
		text = text.substring((cursor + offset).toInt()) ;//.slice( cursor + offset );
		cursor++;
		return result;
